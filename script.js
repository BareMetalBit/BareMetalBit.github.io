...
function renderCatPicker() {
  ...
  var rows = SVC_CATS.map(function(cat) {
    return '<div class="opt-row" id="cat-'+cat+'" onclick="toggleCat(\''+cat+'\')">' +
      '<div class="opt-indicator" id="catind-'+cat+'"></div>' +
      '<div class="opt-text">' +
  ...
  document.getElementById('sheet-actions').innerHTML =
    ...
    '<button class="sheet-btn-primary" id="btn-cats" onclick="startCatDrill()" disabled>CONTINUE →</button>';
}
...
function renderSubOpts() {
  /* always start fresh — no bleed from previous sessions */
  selCats[activeCat] = [];
  ...
  document.getElementById('sheet-label').textContent = '// ' + activeCat + '_options.sh';

  var opts = items.map(function(item, i) {
    return '<div class="opt-row" id="opt-'+i+'" onclick="toggleSubOpt('+i+')">' +
      '<div class="opt-indicator" id="ind-'+i+'"></div>' +
  ...
  document.getElementById('sheet-body').innerHTML =
    '<div class="sheet-cmd">$ ./select <span>--cat=' + activeCat + '</span></div>' +
    '<div class="sheet-h2">' + label + '</div>' +
    '<div class="sheet-sub">Select all that apply' + remainStr + '.</div>' +
    badge +
    '<div class="options-list">' + opts + '</div>';

  var backFn = (isServices && Object.keys(selCats).length > 0)
    ? 'goBackToCats()'
    : (cameFromChoice ? 'goBackToChoice()' : 'closeSheet()');

  document.getElementById('sheet-actions').innerHTML =
    '<button class="sheet-btn-secondary" onclick="' + backFn + '">← BACK</button>' +
    '<button class="sheet-btn-primary" id="btn-sub" onclick="continueFromSub()" disabled>CONTINUE →</button>';
}
...
function renderContact() {
  ...
  var tags = '';
  if (flowType === 'consult') {
    (selCats['consult'] || []).forEach(function(i) {
...
function goBackFromContact() {
  if (flowType === 'consult' || flowType === 'other') {
    sheetStepId = 'subopts';
    renderSheet();
  } else {
    sheetStepId = 'cats';
...
function submitRequest() {
  ...
  Object.keys(selCats).forEach(function(cat) {
    selCats[cat].forEach(function(i) {
      selectedLines.push((SVC_LABELS[cat]||cat) + ' → ' + SVC_ITEMS[cat][i].name);
    });
  });
...
function mobileNavPick(type) {
  ...
  if (type === 'services') openServicesSheet();
  else openSheet('consult');
}
...
/* ══════════════════════════════════
   STATUS PILL — Weekend hours (Sat/Sun 9AM-5PM ET)
══════════════════════════════════ */
function updateStatusPill() {
  var pill = document.getElementById('status-pill');
  if (!pill) return;

  /* Get current time in America/New_York regardless of visitor's local timezone */
  var nowET = new Date(new Date().toLocaleString('en-US', { timeZone: 'America/New_York' }));
  var day    = nowET.getDay();   // 0 = Sunday, 6 = Saturday
  var hour   = nowET.getHours();

  var isWeekend   = (day === 0 || day === 6);
  var isOpenHours = (hour >= 9 && hour < 17);
  var isOnline    = isWeekend && isOpenHours;

  if (isOnline) {
    pill.textContent = '● SERVICES ONLINE';
...
updateStatusPill();
setInterval(updateStatusPill, 60000); /* re-check every minute */
